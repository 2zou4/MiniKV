...
void Wrvoid WriteU64(std::ofstream& out, uint64_t v){
    out.write(reinterpret_cast<const char*>(&v),sizeof(v));
}
...
bool SSTable::BuildFromSkipList(const SkipList& table, const std::string& file_path){
    ...
    for(size_t i=0;i<entries.size();i+=kBlockRecordCount){
        uint64_t block_offset=static_cast<uint64_t>(out.tellp());//out.tellp() — 问文件输出流"你现在写到第几个字节了"，
        //这就是这个 Block 即将开始写入的位置，记下来叫 block_offset
...
bool SSTable:: LoadMetadata(){
    ...
    in.seek(-static_cast<int64_t>(kFooterSize),std::ios::end);
    if(!in.good())
    {
...
bool SSTable::ReadBlock(uint64_t offset, uint64_t size, std::string* out) const{
    ...
    in.seek(static_cast<int64_t>(offset), std::ios::beg);
    out->resize(size);
    if(size>0)
    {
        in.read(&(*out)[0],size);
...
int SSTable::FindCandidateBlock(const std::string& key) const{
    if(indedx_.empty() || key<indedx_[0].first_key){
        return -1;
    }

    inr lo=0, hi=static_cast<int>(indedx_.size())-1;
    int result=-1;

    while(lo<=hi){
        int mid=lo+(hi-lo)/2;
        if(indedx_[mid].first_key<=key){
            result=mid;
            lo=mid+1;
        }else{
            hi=mid-1;
...
// Get
// ──────────────────────────────────────────────
bool SSTable::Get(const std::sstring& key, std::string* value) const{
    if(!valid_){
        return false;
    }
    ...
    while(pos<block_data.size()){
        ...
        std::string cur_key=black_data.substr(pos,key_len);
        pos+=key_len;
